...
void ResourcesManager::LoadAnimationTextures(const pair<string, string> &p,
                                             int frameCount) {
...
map<string, Texture2D> &ResourcesManager::GetTextures() { return textures; }
void ResourcesManager::DrawTexture(const string &textureName, Vector2 position,
  ...
  if (textures.find(textureName) != textures.end()) {
    ...
    Rectangle destRect = {position.x, position.y, texture.width * scale,
