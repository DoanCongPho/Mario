...
public:
  ...
  enum State { WALKING, RUNNING, JUMPING, STANDING, TRANSFORMING };
  void StartTransformation(std::vector<Texture2D> &newWalkingTextures,
                           std::vector<Texture2D> &newJumpingTextures,
                           std::vector<Texture2D> &newRunningTextures);
  Player(const Vector2 &spawnPoint, std::vector<Texture2D> &walkingTextures,
